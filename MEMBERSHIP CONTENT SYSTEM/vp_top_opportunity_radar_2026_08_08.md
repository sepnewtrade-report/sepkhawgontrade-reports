...
```mermaid
flowchart LR
    A["Entry Confirmation Zone ($224.00)"] --> B["Scenario B: Technical Invalidation ($208.66)"]
    A --> C["Scenario A1: Target 1 ($240.00) - Scale Out 50%"]
    C --> D["Scenario A2: Target 2 ($254.57) - Trailing 10-EMA"]
```

### 📌 AMZN Scenario Flowchart:
```mermaid
flowchart LR
    A["Entry Confirmation Zone ($274.50)"] --> B["Scenario B: Technical Invalidation ($255.77)"]
    A --> C["Scenario A1: Target 1 ($295.00) - Scale Out 50%"]
    C --> D["Scenario A2: Target 2 ($311.89) - Trailing 20-EMA"]
```
