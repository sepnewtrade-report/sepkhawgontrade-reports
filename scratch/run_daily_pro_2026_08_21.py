...
def generate_report_pro(template_id, output_filename, date_str):
    ...
    system_instruction = (
        ...
        f"5. ข้อมูลราคาหุ้น ราคาทองคำ ดัชนีทางเทคนิคัล และปัจจัยข่าวสารทั้งหมด ต้องสอดคล้องตรงตามรอบปิดตลาดล่าสุดพฤหัสบดีที่ 20 สิงหาคม 2026 (เวลา US Eastern Time) / รายงานประจำวันที่ {date_str} (เวลาไทย) อย่างเคร่งครัด\n"
    )

    user_prompt = (
        f"วันที่ของรายงาน: {date_str} (เวลาไทย) / US Market Close: พฤหัสบดีที่ 20 สิงหาคม 2026 (เวลา US Eastern Time)\n\n"
...
def generate_script_pro(template_id, report_content, output_filename, date_str):
    ...
    system_instruction = (
        ...
        "ข้อกำหนดในการเขียน:\n"
        "1. เขียนเป็นภาษาไทย ด้วยโทนและรูปแบบการพูดตามที่ระบุใน Audio Prompt Pro อย่างละเอียด (Bloomberg / CNBC / Institutional Morning Brief Style)\n"
